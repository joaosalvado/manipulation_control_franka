#include<my_simple_controllers/torque_control_kdl.h>
#include <pluginlib/class_list_macros.h>  // to allow the controller to be loaded as a plugin


namespace my_simple_controllers {

void TorqueControllerKDL::update(const ros::Time& time, const ros::Duration& period) {
    int kp = 100;
    int kd = 10;

   

   q_d(0) = cos(ros::Time::now().toSec());
...
bool TorqueControllerKDL::init(hardware_interface::EffortJointInterface* hw, 
...
void TorqueControllerKDL::starting(const ros::Time& time) {
...
void TorqueControllerKDL::dummy_init(ros::NodeHandle &root_nh){
...
PLUGINLIB_EXPORT_CLASS(my_simple_controllers::TorqueControllerKDL,
