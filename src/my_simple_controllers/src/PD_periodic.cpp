#include<my_simple_controllers/PD_periodic.h>
#include <pluginlib/class_list_macros.h>  // to allow the controller to be loaded as a plugin

namespace my_simple_controllers {

void PDperiodic::update(const ros::Time& time, const ros::Duration& period) {
   initial_joint_config_[0] = cos(1*ros::Time::now().toSec());
   initial_joint_config_[3] = cos(2*ros::Time::now().toSec());
...
bool PDperiodic::init(hardware_interface::EffortJointInterface* hw, 
...
void PDperiodic::starting(const ros::Time& time) {
    for(auto joint_ : joints_){
        initial_joint_config_.push_back(0.0);
    }
...
PLUGINLIB_EXPORT_CLASS(my_simple_controllers::PDperiodic,
